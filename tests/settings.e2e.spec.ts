...
test.describe("Owner school settings hardening", () => {
  test("navigates settings sections and exposes production hardening controls", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Test Paystack/i })).toBeVisible();

    await page.getByRole("link", { name: /Communication/i }).click();
    await expect(page).toHaveURL(/\/owner\/settings\/communication$/);
    await expect(page.getByRole("heading", { name: "Live Provider Tests" })).toBeVisible();
    await expect(page.getByRole("button", { name: /Test resend/i })).toBeVisible();

    await page.getByRole("link", { name: /Back-up & Data/i }).click();
    await expect(page).toHaveURL(/\/owner\/settings\/backup$/);
    await expect(page.getByRole("heading", { name: "Backup & Restore Execution" })).toBeVisible();
