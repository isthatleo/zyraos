...
export function SignupForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let isAdmin = false
      try {
        const checkRes = await fetch("/api/check-users")
        const { userCount } = await checkRes.json()

        if (userCount === 1 && response.data?.user?.id) {
          // This is the first user, promote to admin
          const promoteRes = await fetch("/api/promote-admin", {
            method: "POST",
          ...
          if (promoteRes.ok) {
            isAdmin = true
            toast.success("🎉 Account created! You have been set as Super Admin.")
          }
        }
      } catch (err) {
        console.error("Failed to check/promote admin status:", err)
      }
