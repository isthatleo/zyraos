...
export default function OwnerPermissionsPage() {
  ...
  const fetchData = React.useCallback(async (silent = false) => {
    ...
    setError("");
    try {
      const response = await fetch(`/api/tenant/owner/permissions?tenant=${encodeURIComponent(tenantSlug)}`, { credentials: "include", cache: "no-store" });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload?.error || "Failed to load owner permissions");
      setData(payload);
      setDrafts(Object.fromEntries(payload.roles.map((role: PermissionRole) => [role.id, role.selectedPermissions])));
    } catch (fetchError) {
      const message = fetchError instanceof Error ? fetchError.message : "Failed to load owner permissions";
      setError(message);
      toast.error(message);
    } finally {
      setLoading(false);
  ...
  async function saveRole(role: PermissionRole) {
    ...
    try {
      const response = await fetch(`/api/tenant/owner/permissions?tenant=${encodeURIComponent(tenantSlug)}`, {
        method: "PATCH",
        credentials: "include",
      ...
      toast.error(saveError instanceof Error ? saveError.message : "Failed to save permissions");
    } finally {
      setSavingRole("");
    }
  }
    ...
    return (
      <Alert variant="destructive" className="rounded-3xl">
        ...
        <AlertDescription className="mt-2 flex items-center justify-between gap-4">
          <span>{error || "The permissions manager could not be loaded."}</span>
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-border/70 bg-card/80 p-6 shadow-sm backdrop-blur">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <Badge className="rounded-full bg-primary/10 text-primary hover:bg-primary/10">User management</Badge>
            <h1 className="mt-4 flex items-center gap-3 text-3xl font-semibold tracking-tight">
              <ShieldCheck className="h-8 w-8 text-primary" />
              Permissions Manager
            </h1>
            <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
              Customize which pages and modules each role can access in {data.school.name}. Changes are saved per role and applied through tenant role permissions.
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "Roles", value: data.summary.roles, detail: "Tenant roles configured", icon: Users },
          { label: "Permission points", value: data.summary.permissions, detail: "Grouped access controls", icon: SlidersHorizontal },
          { label: "Assignments", value: data.summary.assignments, detail: "Saved role permission rows", icon: Check },
          { label: "Users covered", value: data.summary.usersCovered, detail: "Accounts under these roles", icon: ShieldCheck },
        ].map((stat) => {
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-2xl font-semibold">{stat.value.toLocaleString()}</p>
                  </div>
      ...
      <div className="flex flex-col gap-3 rounded-3xl border bg-card/80 p-4 shadow-sm md:flex-row md:items-center md:justify-between">
        <div className="relative w-full md:max-w-md">
          ...
          <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search roles..." className="rounded-2xl pl-9" />
        </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {role.isSystem ? <Badge variant="secondary" className="rounded-full">System</Badge> : null}
                        <Badge variant="outline" className="rounded-full">{roleLabel(role.canonicalRole)}</Badge>
                        {dirty ? <Badge className="rounded-full bg-primary text-primary-foreground">Unsaved</Badge> : null}
                      </div>
                      <CardDescription className="mt-1">{role.description || "Customize access for this tenant role."}</CardDescription>
                      <p className="mt-2 text-xs text-muted-foreground">{role.userCount} users · {selected.length}/{allPermissionIds.length} permissions enabled</p>
                    </div>
                  </div>
                        ...
                        <div className="mb-3 flex items-center gap-2 border-b border-border/70 pb-3">
                          <button
                            ...
                            className={cn(
                              ...
                              groupSelected === group.permissions.length && "border-primary bg-primary text-primary-foreground"
                            )}
                            onClick={() => {
                              ...
                              >
                                <span
                                  className={cn(
                                    ...
                                    checked ? "border-primary bg-primary text-primary-foreground" : "border-muted-foreground/40 text-muted-foreground"
                                  )}
                                >
