...
type BillingPayload = {
  school: { name: string; displayName?: string; logoUrl?: string | null; reportCardWatermarkUrl?: string | null; primaryColor?: string; address?: string; phone?: string; email?: string; website?: string; motto?: string; letterhead?: string; slug: string; type: string; status: string; currencyCode: string };
  generatedAt: string;
  subscription: null | {
    id: string;
    status: string;
...
export default function OwnerBillingPage() {
  ...
  const openDocument = (invoice: PlatformInvoice, print = false) => {
    if (!data) return;
    const opened = openPlatformBillingInvoiceDocument(invoice, data.school, print);
    if (!opened) toast.error("Popup blocked. Allow popups to open the invoice document.");
  };

  const downloadHtml = (invoice: PlatformInvoice) => {
    if (!data) return;
    downloadPlatformBillingInvoiceHtml(invoice, data.school);
  };
  ...
  if (loading) return <div className="space-y-6"><Card className="overflow-hidden border-border/70 bg-card/80 shadow-sm backdrop-blur"><CardContent className="p-6 md:p-8"><Badge className="mb-3 rounded-full bg-primary/10 text-primary hover:bg-primary/10">Owner platform billing</Badge><h1 className="text-3xl font-semibold tracking-tight md:text-4xl">Platform Billing</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Loading subscription usage, platform invoices, exchange-rate conversions, and payment workflow state.</p></CardContent></Card><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <Skeleton key={index} className="h-32 rounded-3xl" />)}</div><Skeleton className="h-96 rounded-3xl" /></div>;
