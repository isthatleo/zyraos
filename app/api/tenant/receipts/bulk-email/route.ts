...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send emails for each receipt
    for (const receiptData of receiptsData) {
      const { receipt } = receiptData

      if (!receipt.recipientEmail) {
        results.push({ id: receipt.id, success: false, error: "No recipient email" })
        failureCount++
        continue
      }
      ...
      try {
        ...
        if (emailResult.success) {
          ...
          results.push({ id: receipt.id, success: false, error: emailResult.error })
          failureCount++
        }
      } catch (error) {
        results.push({ id: receipt.id, success: false, error: "Email sending failed" })
        failureCount++
      }
    }
